...
Observations 269 and 4273 may be influential points, but both of them are smaller than the threshold. Any of these are mutlivariate outliers or severe outliers of a numerical variable when looked in their category of Churn.
...
The results are not perfect, so we see what happens when we remove all multivariate outliers.
...
We cannot compare the BIC nor other measure of fitness between m5 and m5.mout because they do not have the same cardinality. Therefore, we have simply observed the residualPlot, which have gotten worse actually. The conclusion we extract from this fact is that it is better to keep multivariate outliers.
