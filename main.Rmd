...
# the most influent observation are the 269 and 4273 with biggest Cook's distance. But both are not included in the multivariate outlier.
sum(outliers==269)
sum(outliers==4273)
...
# Neither in univariate outlier in tenure and TotalCharges.
...
Observations 269 and 4273 may be influential points, but both of them are smaller than the threshold. Any of these are mutlivariate outliers or severe outliers of a numerical variable when looked in their category of Churn. Hence, we won't remove any of them.
...
The R2 of McFadden isn't excellent, but acceptable. Let's check the residual plots.

```{r Residual plots}
residualPlots(m5)
```

In the residual plot we see that all levels in the model contains severe outliers, except for "Month-to-month" (there is only 1 outlier). However, those points does not seem to affect pearson residuals distributions that are all close to zero as expected.
...
To try to remove those outliers from the categorical variables we remove all multivariate outliers. This gives us an alternative model (m5.mout) that we cannot compare using BIC or other measure of fitness as models does not have the same cardinality. However, the new model does still contains the outliers in the residual plot so we consider that it is better not to remove the multivariate outliers.
...
With the predicted values of the target, we can also study the goodness of fit of the model m5 through the confusion matrix. The most typical measure obtained from this matrix is the accuracy, but in our case it is not trustable because we have unbalanced data. Hence, we resort to the F1-score, a harmonic mean that is less influenced by unbalanced data. We got a value of 56%, which is not great, but it is not a worthless model. This is confirmed by the ROC curve plotted above.

\newpage
# 7. Model interpretation

Finally, let's resume our final model, and try to interpret it.
...
The model is composed by the tenure of costumer, the amound of bill they paid monthly, the kind of contract period that they have with the company, the service they own and then the payment method.

Bear in mind that 0 means "No Churn" and 1 means "Churn Yes", clients who have less probability of leaving the company are those with lowest score. 

Checking the coefficients of m5. The client whose seniority are longer, and contract type with more prolonged period are more comfortable with the company and are less probable to churn.

The coefficient of InternetService equals to "No" is negative, which may indicate that this group of individual prefer to a more basic service but are more stable.

While in the other hand, the individual who own InternetService equals to "Fiber optic", they coefficient is positive. Indicating that they prefer a internet service, but also more flexible to switch the company.

But overall, the most influential variable are Contract equal to "One year" and "Two year". The group of people labeled with this, they probability of Churn reduced significantly.
...
\newpage
# 8. Conclusions
- Our model is simple and easily interpretable
- Our model isn't perfect, but it's fine because it's unbalanced
- Future work: balance data and compare the new model with the actual
\newpage
# Annex
