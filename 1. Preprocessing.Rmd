...
- Ocultar la import de datos y libreria.
...
# There are 344 multivariant outliers, about 5% of the individuals as expect. We'll keep these observation for further analysis.
```

```{r}
# After the report, we remove the newly created variables to  not interfere
...
```{r}
# Check the previous model, but with different transformations like logarithmic.

# First steps is to convert the 0 to 1e-6. By doing this, we'll be able to implement the boxtidewell method
train_positive = replace(train, train == 0, 1e-6)

train_positive$Churn == ifelse(train$Churn == "False", 0, 1)
