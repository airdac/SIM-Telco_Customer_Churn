...
# and {TotalCharges}, as the second is simply the product of first set.
...
# Hence, we cannot remove any of them.
Anova(m0.set1, test = "LR")

m0.set2 = glm (Churn ~ TotalCharges, data = train, family = binomial)

# Checking the Akaike's criterion, the set {tenure, MonthlyCharges} 
...
```{r}
# Check the possible transformation for the previous model

m0.log = glm (Churn ~ tenure + log(MonthlyCharges), data = train, family = binomial)
m0.sqrt = glm (Churn ~ tenure + sqrt(MonthlyCharges), data = train, family = binomial)

# AIC shows that the model without transformation is better.
AIC (m0.set1, m0.log, m0.sqrt)
```

```{r}
# Adding up categorical variables {c.tenure}
...
```{r}
# Adding up categorical variables {Contract}
m2 = glm (Churn ~ tenure + TotalCharges + Contract, 
...
Based on the Catdes, the most related variable is OnlineSecurity, but we have figured out
in the Profiling part that the InternetService has bigger impact on the Churn.
The InternetService with "No" level is much more important than OnlineSecurity.
```{r}
# Adding up categorical variables {InternetService}
m3 = glm (Churn ~ tenure + TotalCharges + Contract + InternetService, 
          data = train, family = binomial)

m3.i = glm (Churn ~ tenure + TotalCharges + Contract + OnlineSecurity, 
          data = train, family = binomial)
summary(m3)
...
```{r}
# Adding up categorical variables {OnlineSecurity}
m4 = glm (Churn ~ tenure + TotalCharges + Contract + InternetService + OnlineSecurity, 
          data = train, family = binomial)
...
```{r}
# Adding up categorical variable {PaymentMethod}
m5 = glm (Churn ~ tenure + TotalCharges + Contract + InternetService + PaymentMethod, 
          data = train, family = binomial)

m5.i = glm (Churn ~ tenure + TotalCharges + Contract + InternetService + OnlineSecurity + TechSupport + PaymentMethod, 
          data = train, family = binomial)

BIC(m5, m5.i)

# The model m5.i has NA, as for the people who don't have internet service, both
# TechSupport and OnlineSecurity are not applicable.
summary(m5.i)
summary(m5)
