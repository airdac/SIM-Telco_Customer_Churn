...
# Checking the Anova test, there is no variable removed, which means that all of then are relevant.
Anova (m0.numerical)
```
...
boxcox(Churn ~ tenure + TotalCharges, data = train_positive)
...
m1 = glm (Churn ~ tenure + log(TotalCharges), data = train, family = binomial)
```
