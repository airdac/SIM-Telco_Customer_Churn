ValorTestXnum <- function(Xnum,P){
  ...
  for(c in 1:length(levels(P))){if (pxk[c]>0.5){pxk[c]<-1-pxk[c]}}
  return (pxk)
}
...
ValorTestXquali <- function(P,Xquali){
  ...
  for(c in 1:length(levels(P))){for (s in 1:length(levels(Xquali))){if (pzkj[c,s]> 0.5){pzkj[c,s]<-1- pzkj[c,s]}}}
  return (list(rowpf=pf,vtest=zkj,pval=pzkj))
}

profiling <- function(df, P, nameP) {
  dades <- df
  K <- dim(dades)[2]
  n <- dim(dades)[1]
  dd <- df
